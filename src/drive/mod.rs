...
use ex::{ExCmd, parse_ex_command};
use input::{
    InputDrainResult, SCROLL_STEP, apply_input_drain, drain_input_edits, pane_dims,
    poll_user_quit_only,
};
...
fn load_task(dir: &Path, caps: &aden::AdenCaps) -> Task {
    let bare = || Task {
        ...
        context: if bare_mode() {
            ...
            let mut ctx = AGENT_PREAMBLE_BASE.to_string();
            if caps.available {
                ctx.push_str(
                    "aden is on PATH — use aden_grep, aden_locate, aden_asm, aden_understand, \
aden_ask for structural context; call aden_tools to list capabilities.\n\n",
                );
            }
            Some(ctx)
        },
...
mod tests {
    ...
    #[test]
    fn load_task_nudges_aden_tools_when_available_ungated() {
        let _env = crate::test_env::lock();
        let old = std::env::var("COXN_TASK_NAME").ok();
        let old_bare = std::env::var("COXN_BARE").ok();
        unsafe {
            std::env::remove_var("COXN_TASK_NAME");
            std::env::remove_var("COXN_BARE");
        }
        ...
        let task = load_task(std::path::Path::new("."), &caps);
        let ctx = task.context.expect("context");
        assert!(
            ctx.contains("aden_grep"),
            "expected aden tool nudge in ungated context: {ctx}"
        );
        unsafe {
            match old {
                Some(v) => std::env::set_var("COXN_TASK_NAME", v),
                None => std::env::remove_var("COXN_TASK_NAME"),
            }
            match old_bare {
                Some(v) => std::env::set_var("COXN_BARE", v),
                None => std::env::remove_var("COXN_BARE"),
            }
        }
    }
}
